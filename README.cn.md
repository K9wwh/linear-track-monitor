...
- **刚起步的头几拍 `Eta` 可能偏大**:它除的是**平滑后**的 `Speed`,而滑动平均窗口里这时可能还压着停机期间留下的陈旧零速样本 (stale zero-speed samples) —— 控件加载期间引擎一直在采样,轴不动时窗口就被 0 填满(引擎采样快于位置数据回传节奏、采到零位移样本时同理)。这些 0 把均值拉低,`Speed` 因而报得偏小、ETA 随之偏大,直到它们被挤出窗口为止,大约需要 `SpeedSmoothingWindow` 拍。(窗口只是"还没填满"并**不会**造成偏差:均值是按实际收集到的样本数归一化的。)若界面一动就显示 ETA,会闪一下偏大的数字 —— 想避开就把 `SpeedSmoothingWindow` 调小,或在 `Status` 转 `Running` 后先压住 ETA,等旧零样本被挤出窗口(约 `SpeedSmoothingWindow` 拍)再显示。
