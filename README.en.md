...
- **`Eta` can read high for the first few ticks after motion starts**: it divides by the **smoothed** `Speed`, and the moving-average window may still be holding stale zero-speed samples from the period when the axis was standing still — the engine keeps sampling for as long as the control is loaded, so an idle axis fills the window with zeros (the same thing happens if the engine samples faster than the position source updates, which injects zero-delta samples). Those zeros drag the average down, so `Speed` under-reports the true speed and the ETA reads pessimistically high until they are pushed out of the window, which takes roughly `SpeedSmoothingWindow` ticks. (A window that has merely not filled up yet is *not* a problem: the average is normalized by the number of samples actually collected.) If the UI shows the ETA the moment motion starts, an over-long number will flash up — to avoid it, lower `SpeedSmoothingWindow`, or hold the ETA back after `Status` turns `Running` until the stale zeros have cleared (about `SpeedSmoothingWindow` ticks).
